---
title: SLOW_QUERY
summary: `SLOW_QUERY` INFORMATION_SCHEMA テーブルの使い方を学びます。

# SLOW_QUERY

`SLOW_QUERY` テーブルは、現在のノードの遅いクエリ情報を提供します。これは、TiDB の遅いクエリログファイルの解析結果です。このテーブルの列名は遅いクエリログのフィールド名に対応しています。

> **注意:**
>
> このテーブルは[TiDB サーバーレス](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-serverless)クラスターでは利用できません。
...
## CLUSTER_SLOW_QUERY テーブル

`CLUSTER_SLOW_QUERY` テーブルは、クラスター内のすべてのノードの遅いクエリ情報を提供します。これは、TiDB の遅いクエリログファイルの解析結果です。`CLUSTER_SLOW_QUERY` テーブルのテーブルスキーマは `SLOW_QUERY` テーブルと異なり、 `CLUSTER_SLOW_QUERY` には `INSTANCE` 列が追加されています。`INSTANCE` 列は遅いクエリの行情報の TiDB ノードアドレスを表します。
...
クラスターシステムテーブルが問い合わされると、TiDBはすべてのノードからデータを取得するのではなく、関連する計算を他のノードにプッシュダウンします。実行計画は次のようになります:
...
前述の実行計画では、`user = u1` の条件は他の(`cop`) TiDBノードにプッシュダウンされ、集約演算子もプッシュダウンされています(グラフ内の`StreamAgg`演算子)。

現在、システムテーブルの統計情報が収集されていないため、時々いくつかの集計演算子がプッシュダウンされず、実行が遅くなることがあります。この場合、手動でSQL HINTを指定して集計演算子をプッシュダウンすることができます。例えば:
