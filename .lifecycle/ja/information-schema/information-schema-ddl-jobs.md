---
title: DDL_JOBS
summary: `DDL_JOBS`情報スキーマテーブルについて学びます。

# DDL_JOBS

`DDL_JOBS`テーブルは、[`ADMIN SHOW DDL JOBS`](/sql-statements/sql-statement-admin-show-ddl.md) コマンドに対する`INFORMATION_SCHEMA`インターフェースを提供します。TiDBクラスタ全体でのDDL操作の現在のステータスと短い履歴を提供します。
