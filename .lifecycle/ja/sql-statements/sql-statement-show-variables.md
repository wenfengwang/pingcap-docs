---
title: SHOW [GLOBAL|SESSION] VARIABLES | TiDB SQLステートメントリファレンス
summary: TiDBデータベースのSHOW [GLOBAL|SESSION] VARIABLESの使用方法の概要です。
aliases: ['/docs/dev/sql-statements/sql-statement-show-variables/','/docs/dev/reference/sql/statements/show-variables/']
---

# SHOW [GLOBAL|SESSION] VARIABLES

このステートメントは、`GLOBAL`または`SESSION`のスコープの変数のリストを表示します。スコープが指定されていない場合、デフォルトの`SESSION`スコープが適用されます。
...
すべてのTiDB固有の変数をリストします。詳細については、[System Variables](/system-variables.md)を参照してください。
...
TiDBの`SHOW [GLOBAL|SESSION] VARIABLES`ステートメントは、MySQLと完全に互換性があります。互換性の違いがある場合は、[バグを報告してください](https://docs.pingcap.com/tidb/stable/support)。
