---
title: TiDB TPC-H 50G パフォーマンステストレポート V2.0
aliases: ['/docs/dev/benchmark/benchmark-tpch/','/docs/dev/benchmark/tpch/']
---

# TiDB TPC-H 50G パフォーマンステストレポート

## テスト目的

このテストは、OLAPシナリオでTiDB 1.0とTiDB 2.0のパフォーマンスを比較することを目的としています。

> **注意:**
>
> 異なるテスト環境では異なるテスト結果になる可能性があります。
...
| マシン IP      | オペレーティング システム     | カーネルバージョン     | ファイルシステムタイプ |
|--------------|------------------------|------------------------------|--------------|
...
![TPC-H クエリ結果](/media/tpch-query-result.png)

以下に注意すべき点があります:

- 上記の図では、オレンジのバーはリリース 1.0のクエリ結果を、青のバーはリリース 2.0のクエリ結果を表しています。y軸はクエリの処理時間（秒単位）を表し、短いほど速いです。
- クエリ 15 は VIEW が現在、TiDB 1.0 または 2.0 のいずれもサポートされていないため、"NaN" とタグ付けされています。将来のリリースで VIEW のサポートを提供する予定です。
- TiDB 1.0 列のクエリ 2、17、および 19 は、これらのクエリに対する TiDB 1.0 の結果が返されなかったため、"NaN" とタグ付けされています。
- TiDB 1.0 列のクエリ 5、7、18、および 21 はメモリ消費が高すぎたため、"OOM" とタグ付けされています。